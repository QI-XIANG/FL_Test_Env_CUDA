...
class FedBulyanAdvanced(Server):
    ...
    def bulyan(self, weights, n_attackers):
        ...
        # Dynamic clustering based on the number of clients and variance in updates
        n_clusters = max(1, len(client_updates) // 5)  # At least one cluster, but no more than a small number
        kmeans = KMeans(n_clusters=n_clusters, random_state=0).fit(client_updates)
        
        # Get the centroid of each cluster
        centroids = kmeans.cluster_centers_
        
        # Find the client whose update is closest to the centroid (central model)
        closest_client_idx = np.argmin([self.euclidean_distance(centroid, update) for centroid, update in zip(centroids, client_updates)])

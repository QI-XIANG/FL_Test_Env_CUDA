...
class FedRobustBulyan(Server):
    ...
    def robust_bulyan(self, weights, n_attackers, kmeans_clusters=3):
        """
        Improved Bulyan with enhanced selection and clustering for higher robustness.
        """
        ...
        # Dynamic clustering with multiple clusters for better resistance to poisoners
        kmeans = KMeans(n_clusters=kmeans_clusters, random_state=0).fit(client_updates)
        cluster_centroids = kmeans.cluster_centers_

        # Find the closest client to each cluster centroid and select the one closest to the overall centroid
        centroids_distances = [self.euclidean_distance(centroid, np.mean(client_updates, axis=0)) for centroid in cluster_centroids]
        
        best_centroid_idx = np.argmin(centroids_distances)
        cluster_members = np.where(kmeans.labels_ == best_centroid_idx)[0]

        # Return the client whose update is closest to the centroid of the best cluster
        centroid = cluster_centroids[best_centroid_idx]
        closest_client_idx = np.argmin([self.euclidean_distance(centroid, client_updates[idx]) for idx in cluster_members])
        
        return krum_selected_indices[cluster_members[closest_client_idx]]
    ...
    def train(self):
        ...
        with mlflow.start_run(run_name=f"noniid_wbn_{self.num_clients * self.poisoned_ratio}_RobustBULYAN"):
            ...
            for i in range(self.global_rounds + 1):
                ...
                bulyan_client_index = self.robust_bulyan(clients_weight, int(self.num_join_clients * self.poisoned_ratio))
